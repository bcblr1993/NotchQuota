...
enum GoogleCredentials {
    static func read() throws -> GoogleCredential {
        ...
        guard let object = try JSONSerialization.jsonObject(with: payload) as? [String: Any], let token = object["token"] as? [String: Any], let access = token["access_token"] as? String, let refresh = token["refresh_token"] as? String else { throw QuotaError.message("Antigravity 登录存储格式已变化") }
        return GoogleCredential(access: access, refresh: refresh, expiry: QuotaParser.date(token["expiry"]))
    }
    static func clientCandidates() throws -> [(String, String)] {
        let url = try Installation.antigravityServer()
