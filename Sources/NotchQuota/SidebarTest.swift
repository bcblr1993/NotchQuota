    ...
    func runSidebarTest() {
        Task { @MainActor in
            ...
            if let screen = NSScreen.main {
                ...
                check(bar.collapsed && bar.panel.frame.width == 60 && bar.clock == nil && bar.idleWork == nil, "idle floating strip becomes quiet compact emblem")
                ...
                for style in SidebarAppearance.allCases {
                    ...
                    for right in [false, true] {
                        ...
                        bar.exitBar(); bar.setCollapsed(true); await wait(0.4)
                        capture("style-" + style.rawValue + (right ? "-right" : "-left"), view: bar.surface)
                        bar.enterBar()
                        check(!bar.collapsed && bar.expandWork == nil, "style expands immediately without dwell: " + style.rawValue)
                        await wait(0.4)
                        check(screen.visibleFrame.contains(bar.panel.frame), "expanded style stays on display: " + style.rawValue)
                ...
                check(lastAttempt == requests, "appearance changes never request quota")
                bar.setAppearance(.capsule)

                bar.enterBar(); await wait(0.12)
                check(!bar.collapsed && bar.expandWork == nil, "brief hover opens immediately")
                capture("sidebar-emblem-peek", view: bar.surface)
                bar.exitBar(); await wait(0.5)
                check(!bar.collapsed && bar.expandWork == nil, "pointer exit keeps normal idle collapse delay")
                ...
                bar.exitBar(); bar.scheduleIdleCollapse(after: 0.1); await wait(0.6)
                check(bar.docked && bar.collapsed && bar.panel.frame.width == 60 && abs(bar.panel.frame.maxX - screen.visibleFrame.maxX) < 1, "edge docking shows complete icon")
                capture("sidebar-edge-handle", view: bar.surface)
                check(!bar.hasEmblemAnimations, "idle emblem stays static")
                suppressMotion = true; updateSidebar()
                check(!bar.hasEmblemAnimations, "reduced motion stops idle emblem animations")
                suppressMotion = false; updateSidebar()
                bar.enterBar(); await wait(0.4)
                check(!bar.collapsed && bar.panel.frame.width == 52, "edge icon opens panel without peeking")
                capture("sidebar-emblem-edge-peek", view: bar.surface)
                bar.exitBar()
                bar.setCollapsed(false); await wait(0.4)
