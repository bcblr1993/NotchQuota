...
- **小幽灵提醒**：每 10 分钟轮换检查一个账号，查询成功后冒出思考泡泡，约 5 秒后淡出；失败或旧数据跳过。
...
以下图片来自 **v0.1.16 应用原生界面渲染**，不是概念设计图。额度、账号名称和时间为演示数据；透明区域会随 GitHub 的明暗主题显示不同底色。
...
头像旁直接显示额度，悬停查看详情，点击可固定。可自由摆放，也可吸附左右边缘；鼠标跨屏不会带走它。收起外观提供原生胶囊、磨砂圆球、迷你仪表和小幽灵。
...
收起外观可在「右键 → 设置与更新 → 收起外观」选择，默认原生胶囊；即时生效并记住选择。四种外观共用拖动、贴边、移入展开和账号详情；小幽灵提醒会在提醒前查询额度，其他外观不启用该提醒任务。磨砂圆球使用静态渐变，小幽灵自由悬浮时直立，贴边时向桌面方向倾斜探出；仅当前样式参与渲染。
...
小幽灵思考提醒单独每 10 分钟检查一次：提醒前主动查询，复用同账号正在进行的请求，只有本次查询成功且数据有效才展示。各账号依次尝试，避免同时发起一批提醒查询。
