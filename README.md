...
- 只显示图标和剩余额度，无操作 **15 秒后收起，仅保留刘海额度线**；鼠标移到屏幕顶部中央唤出。
...
显示/隐藏、展开/收起和图标切换使用约 0.2 秒的原生过渡；系统开启“减少动态效果”或低电量模式时立即切换。界面没有逐帧轮询，三枚图标在内存中复用。

只显示本机已安装的 Codex、Claude、Antigravity 桌面应用（ChatGPT 不算 Codex）。图标统一为 16 点视觉尺寸。只安装一个时点击图标不会切换；都未安装时不显示窗口、图标或额度。安装检测在启动、唤出及每 3 分钟刷新时执行；检测到新安装应用不会自行弹出，移入顶部热点即可显示。应用无需运行，但查询额度仍需已有有效登录。

隐藏后的 2.1 点高饱和度细线沿刘海两侧和底部显示剩余比例，优先显示 Codex，未安装则选择 Claude，再其次 Antigravity。没有任何应用时完全不显示。灰线代表未知或旧数据；普通无刘海屏使用顶部居中的短线。细线以约 4.8 秒一轮缓慢呼吸，使用单个 Core Animation 透明度动画，无逐帧定时器和动态模糊。展开详情、屏幕休眠、低电量或开启“减少动态效果”时停止呼吸；未知/过期额度保持静态灰色。细线不会拦截鼠标。
