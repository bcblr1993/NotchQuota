...
收起外观可在「右键 → 设置与更新 → 收起外观」选择，默认原生胶囊；即时生效并记住选择。四种外观共用拖动、贴边、移入展开和账号详情，不创建额外刷新任务。磨砂圆球使用静态渐变，小幽灵显示倾斜探头的身体；仅当前样式参与渲染。
